...
/// struct for typed errors of method `get_fetch_collection1234567891011121314`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetFetchCollection1234567891011121314Error {
...
/// struct for typed errors of method `get_fetch_resource1234567891011121314`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetFetchResource1234567891011121314Error {
    Status400(crate::models::Error406),
    Status404(crate::models::Error406),
    Status500(crate::models::Error406),
    UnknownValue(serde_json::Value),
    ...
    pub async fn get_fetch_collection1234567891011121314(configuration: &configuration::Configuration, id: f64) -> Result<crate::models::FetchCollectionresponse15, Error<GetFetchCollection1234567891011121314Error>> {
        let client = &configuration.client;

        let uri_str = format!("{}/site-announcements", configuration.base_path);
        ...
        if status.is_success() {
            ...
            let entity: Option<GetFetchCollection1234567891011121314Error> = serde_json::from_str(&content).ok();
            let error = ResponseContent { status, content, entity };
            Err(Error::ResponseError(error))
        }
    }

    pub async fn get_fetch_resource1234567891011121314(configuration: &configuration::Configuration, id: f64) -> Result<crate::models::FetchResourceresponse15, Error<GetFetchResource1234567891011121314Error>> {
        let client = &configuration.client;

        let uri_str = format!("{}/site-announcements/{id}", configuration.base_path, id=id);
        let mut req_builder = client.get(uri_str.as_str());
        ...
        if status.is_success() {
            ...
            let entity: Option<GetFetchResource1234567891011121314Error> = serde_json::from_str(&content).ok();
            let error = ResponseContent { status, content, entity };
            Err(Error::ResponseError(error))
        }
    }
